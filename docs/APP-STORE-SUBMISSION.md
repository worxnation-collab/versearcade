...
4. Codemagic → **Teams/User settings → Integrations → App Store Connect → Add key**.
   - Name it **exactly** `VerseArcade ASC` (this matches `integrations:` in `codemagic.yaml`).
...
- ✅ **No external payment links** for digital goods; v1 has no paid features at all,
   so no IAP is required for this release.
