...
### Auto-submit is ON — what that changes

`publishing.app_store_connect.submit_to_app_store` is now `true`. Codemagic uploads to
TestFlight and then submits the build for App Store review by itself, with
`release_type: MANUAL` (Apple approves; the app goes live when *you* press "Release")
and `cancel_previous_submissions: true` (a re-run replaces a submission still sitting
in "Waiting for Review" instead of being refused).

Codemagic *submits* the version. It does not *fill it in*. So before you push:

- [ ] The version record exists in App Store Connect (**+ Version or Platform** →
      `1.2.0`) with description, keywords, support/privacy URLs, age rating and the
      App Privacy answers complete.
- [ ] **"What's New" pasted in** from `APP-STORE-LISTING.md`.
- [ ] Screenshots uploaded (6.7" iPhone, 1290 × 2796, at least 3).
- [ ] **Demo account credentials filled into the review notes.** Mandatory since the
      account wall — a reviewer without them sees four padlocked tabs.
- [ ] Export compliance: handled in the binary (`ITSAppUsesNonExemptEncryption=false`
      is written into Info.plist by the build), so no "Missing Compliance" gate.

An incomplete version record fails the publishing step *after* a successful 20-minute
archive — the same expensive shape as the version-train rejection above.

> **The one thing auto-submit skips is the device smoke test**, and crash-on-launch is
> the top avoidable rejection. The TestFlight upload still happens first, so you can
> install the build while review is queued. If it crashes, cancel the submission in
> App Store Connect — you have hours, not minutes, but you do have to look.
