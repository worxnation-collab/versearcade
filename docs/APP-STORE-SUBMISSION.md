...
**1.0 is live.** That changes the rules: an approved version's "train" closes, and
App Store Connect refuses any further upload carrying that version number. Builds
22 and 23 died exactly here — signed fine, archived fine, then the upload came back
with `90062` ("must contain a higher version than that of the previously approved
version [1.0]") and `90186` ("the train version '1.0' is closed for new build
submissions"). Nothing was wrong with the key, the certificate, or the profile.

So every release now starts with a version bump:
...
4. In App Store Connect, create the new version record (**+ Version or Platform**),
   attach the processed build, fill "What's New", and submit.
