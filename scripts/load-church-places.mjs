...
// The refreshes ride out as the LAST FILE rather than as a line printed at the
// end of a long run, and that is the whole point of them being here.
//
// Two tables are DERIVED from church_places and rebuilt only when asked:
// `churches.name` (refresh_church_names, 0091) and `church_cities` (0114).
// Both fail in the quietest possible way when a load skips them — a renamed
// congregation keeps its old name, and a newly loaded region's towns are simply
// absent from the city picker — and neither errors, so nobody finds out.
// Printing "remember to run these" is what this file did, and a printed
// instruction at the bottom of a multi-gigabyte download is an instruction that
// gets scrolled past.
//
// So they are `NNN_refresh.sql`, and "apply the files in order" now performs
// them. It sorts last because it is numbered last, it is idempotent, and it is
// the only file here that is not INSERTs.
const refreshName = `${String(++fileNo).padStart(3, '0')}_refresh.sql`
writeFileSync(
  join(OUT_DIR, refreshName),
  `-- Generated by scripts/load-church-places.mjs — do not hand-edit.
-- APPLY THIS LAST, every time. Both tables below are derived from
-- church_places and rebuilt only when asked; skipping this leaves a renamed
-- church wearing its old name and a whole region missing from the city
-- picker, with nothing failing anywhere to say so.
select public.refresh_church_names();
select public.refresh_church_cities();
`,
)
files.push(refreshName)

console.log(`\nWrote ${files.length} file(s) to ${OUT_DIR}/`)
console.log(`Apply them IN ORDER — the last one (${refreshName}) does the refreshes.`)
console.log('\nOne extra step after the FIRST load only:')
console.log('  select public.link_church_places();')
