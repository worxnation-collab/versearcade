...
// A church, drawn the same way everywhere it appears.
//
// Extracted from ChurchDetailSheet the moment /church/:id wanted the same
// thing on a page instead of in a sheet. It is the church's version of the
// one-component-per-world rule the little worlds already follow (see the table
// in CLAUDE.md): a second copy of this would drift, and both copies would look
// right on their own, which is how that kind of bug survives for months.
//
// The sheet renders it with everything on. The public page turns off the three
// things a stranger may not have — the congregation's NAMES, the "Add info"
// queue and the yard read — and each of those is a prop rather than a check
// inside, so the surface can't decide to be public by accident.
export function ChurchPageBody({
  page,
  loading,
  onClose,
  named = true,
  ...
  page: ChurchPage
  loading: boolean
  /** The sheet's Close pill. A page has no Close — it has a back button. */
  onClose?: () => void
  /**
   * Name the congregation.
   *
   * False on /church/:id signed out, where `public_church_page` (0074) returns
   * no usernames at all. The crowd is still drawn and still counted; a list of
   * real names against a named physical address, readable by anyone who follows
   * a link, is a different product from the same list shown to somebody already
   * inside — which is what `profiles` being readable by the `authenticated`
   * role and nobody else (0002) has always said.
   */
  named?: boolean
  /** `submit_church_info_request` is authenticated-only (0050). */
  canAsk?: boolean
  /** `church_yard_json` is authenticated-only (0061), so the public page has no
   *  landscaping to draw and shouldn't fire the call to find that out. */
  withYard?: boolean
  /** Sits under the page — the public screen's way in. */
  ...
  return (
    <>
      ...
      {named ? (
        ...
        <Crowd total={memberTotal} loading={loading} />
      )}

      {canAsk && <InfoSection page={page} loading={loading} />}
      {footer}
      <div style={{ height: 8 }} />
    </>
  )
...
// The congregation, counted and not named.
//
// What the public page shows where the signed-in page shows "Who plays here".
// It is deliberately not a shortened roster or a blurred one — there is simply
// no name in the payload to show (0074). The head count stays, because a head
// count is a fact about a place and it is already on the board; it is not a
// number on anybody.
function Crowd({ total, loading }: { total: number; loading: boolean }) {
  if (loading && !total) return null
  return (
    <div className="card" style={{ marginBottom: 14 }}>
      <b style={{ fontFamily: 'var(--font-display)', fontSize: 16 }}>Who plays here</b>
      <p className="dim" style={{ margin: '6px 0 0', fontSize: 13.5, lineHeight: 1.55 }}>
        {total === 0
          ? 'Nobody yet — this church is on the board, but no one has picked it as theirs.'
          : total === 1
            ? 'One player calls this church theirs. That’s them on the grass.'
            : `${total.toLocaleString()} players call this church theirs — that’s them on the grass. Their names are here once you have an account.`}
      </p>
    </div>
  )
}
