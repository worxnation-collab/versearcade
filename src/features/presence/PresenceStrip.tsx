...
export function PresenceStrip() {
  ...
  return (
    <div className="card" style={{ padding: 14, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', height: 30, overflow: 'hidden', maskImage: 'linear-gradient(90deg, transparent, #000 12%, #000 88%, transparent)' }}>
        <motion.div
          animate={{ x: ['0%', '-50%'] }}
          transition={{ repeat: Infinity, ease: 'linear', duration: 22 }}
          style={{ display: 'flex', gap: 10, whiteSpace: 'nowrap', position: 'absolute' }}
        >
          {feed.map((f, i) => (
            <span key={i} className="pill" style={{ fontSize: 12, padding: '4px 10px' }}>
