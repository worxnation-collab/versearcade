...
export const useCollection = create<CollectionState>((set, get) => ({
  ...
  async openChest(dropDate) {
    ...
    if (isOnline() && supabase) {
      ...
      const res = data as ChestResult
      const owned = get().owned
      set({
        lastChestOn: dropDate,
        owned: res.key && !owned.includes(res.key) ? [...owned, res.key] : owned,
      })
      return res
    }
    // Guest (offline) chest — mirror the server's odds locally.
    ...
    return { alreadyOpened: false, key, rarity }
  },
}))
